...
// import { getItem } from "../service/local-storage.service";
...
export class BaseContainer extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.logged !== this.props.logged && !this.props.logged) {
      ...
      // histroy.push('/auth/login');
    }
  }
  checkUser = () => {
    const { history } = this.props; //checkUser, setUserTemp,
    ...
    // 로그인을 유지하기 위함.
    // let { id, userId, ttl, created } = getItem("AUTH.USERINFO");
    // if (id) {
    //   setUserTemp({ id, userId, ttl, created });
    //   return;
    // }
